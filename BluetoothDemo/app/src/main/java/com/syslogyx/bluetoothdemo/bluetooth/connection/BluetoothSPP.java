...
public class BluetoothSPP {
    ...
    public boolean isBluetoothConnected() {
        ...
        if (mChatService != null) {
            ...
            if (mChatService.getState() == BluetoothStateConstants.STATE_CONNECTED) {
                return true;
            } else {
                return false;
            }
    ...
    public void startService(boolean isAndroid) {
        if (mChatService != null) {
            if (mChatService.getState() == BluetoothStateConstants.STATE_NONE) {
    ...
    private final Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case BluetoothStateConstants.MESSAGE_WRITE:
                    break;
                case BluetoothStateConstants.MESSAGE_READ:
                    ...
                    break;
                case BluetoothStateConstants.MESSAGE_DEVICE_NAME:
                    mDeviceName = msg.getData().getString(BluetoothStateConstants.DEVICE_NAME);
                    mDeviceAddress = msg.getData().getString(BluetoothStateConstants.DEVICE_ADDRESS);
                    if (mBluetoothConnectionListener != null)
                        mBluetoothConnectionListener.onDeviceConnected(mDeviceName, mDeviceAddress);
                    isConnected = true;
                    break;

                case BluetoothStateConstants.MESSAGE_TOAST:
                    Toast.makeText(mContext, msg.getData().getString(BluetoothStateConstants.TOAST)
                            , Toast.LENGTH_SHORT).show();
                    break;
                case BluetoothStateConstants.MESSAGE_STATE_CHANGE:
                    if (mBluetoothStateListener != null)
                        mBluetoothStateListener.onServiceStateChanged(msg.arg1);
                    if (isConnected && msg.arg1 != BluetoothStateConstants.STATE_CONNECTED) {
                        if (mBluetoothConnectionListener != null)
                    ...
                    if (!isConnecting && msg.arg1 == BluetoothStateConstants.STATE_CONNECTING) {
                        isConnecting = true;
                    } else if (isConnecting) {
                        if (msg.arg1 != BluetoothStateConstants.STATE_CONNECTED) {
                            if (mBluetoothConnectionListener != null)
    ...
    public void connect(Intent data) {
        String address = data.getExtras().getString(BluetoothStateConstants.EXTRA_DEVICE_ADDRESS);
...
//        System.out.println(">>>>>> Disable bluetooth : " + mBluetoothAdapter.disable());
    }
    ...
    public void send(String data, boolean CRLF) {
        if (mChatService.getState() == BluetoothStateConstants.STATE_CONNECTED) {
            ...
            Log.d("BluetoothSPP", "send--" + data);
        }
        Log.d("BluetoothSPP", "send--ACK FAILED" + data);
